...
`src/language_components/libraries/functions.py` uses `getattr` for library function and constant dispatch; `src/language_components/method_calling/functions.py` uses it for object functions and constants. Other `getattr` calls in `language_components/functions/functions.py`, `language_components/functions/utils.py`, and `src/lib/KafeMACHINE/BaseMachine.py` inspect English Python attributes (`signature`, `_name`, `_is_fitted`) and are not KAFE name translations. `DataFrame.query(query_str)` calls the generated KAFE lexer/parser on a KAFE expression, injects column values into visitor scope via `assign_variable`, and evaluates the tree. Its method name and expression grammar remain unchanged; the internal helper import changed with Task 2.
...
|---|---|---|
| `construir_tipo_lista` | `build_list_type` | Defined and recursively called in `src/TypeUtils.py`; used there to construct type constants and imported/called by `src/lib/KafePLOT/functions.py` for `bar` and `pie` signatures. Tasks 2 and 3 must update both modules together. |
| `es_misma_dimension` | `has_same_dimensions` | Defined in `src/lib/KafeNUMK/utils.py`; imported and called by `src/lib/KafeNUMK/functions.py` in `add` and `sub`. It compares outer lengths and each corresponding row length. Task 3. |
| `es_uniforme` | `is_uniform_matrix` | Defined in `src/lib/KafeNUMK/utils.py`; imported and called by `src/lib/KafeNUMK/functions.py` in `mul`, `inv`, and `dot_matrix`. It checks equal row lengths and accepts an empty matrix. Task 3. |
| `operar_matrices` | `apply_matrix_operation` | Defined in `src/lib/KafeNUMK/utils.py`; imported and called by `src/lib/KafeNUMK/functions.py` in `add` and `sub` to apply the supplied operation element by element. Task 3. |
...
| `matriz_cualquiera_t` | `any_matrix_type` | `KafeNUMK/functions.py`, `KafePARDOS/DataFrame.py`, `KafeMACHINE/preprocessing/SimpleImputer.py` |
| `lista_cadenas_t` | `string_list_type` | `KafeGESHA/functions.py`, `KafeGESHA/core/model.py`, `KafePARDOS/DataFrame.py`, `KafeMACHINE/preprocessing/OneHotEncoder.py`, `OrdinalEncoder.py` |
...
| `func_nombre` | `function_name` | `global_utils.py` decorator keyword lookup; keyword callers in `language_components/functions/functions.py` and `KafePLOT/functions.py` |
...
Task 3 review also removed legacy activation selector strings. `sigmoide` → `sigmoid`, `tangente` → `tanh`, `identidad` → `identity` (or `linear`), and `escalon`/`escalonada` → `step`. These are breaking input changes for callers of the Python activation loader; no Spanish selector aliases remain. No first-party fixture or example used those old selector strings.
...
The Task 3 review changed the plotted SVG label `Leyenda` to `Legend`. Its seven directly corresponding SVG expected files were updated: six `tests/KafePLOT/grafico_*.svg` fixtures (`plot_dense_data`, `plot_exponential_growth`, `plot_mixed_positive_negative`, `plot_sine_wave`, `plot_sparse_data`, `plot_zigzag_pattern`) and `tests/KafeGESHA/multiclass/grafico_neural_network_multiclass.svg`. Other fixture content remains for Task 4.

`importStmt` first recognizes the unchanged registry keys. For user modules it searches `<current KAFE program directory>/<module>.kf`, then `<directory of imports/functions.py>/<module>.kf`, then its parent `<language_components>/<module>.kf`. Moving the module changes those latter two fallback directories; preserve this search behavior intentionally and document the resulting paths in import diagnostics. The former `tests/import/matematica.kf` is now `tests/import/math_module.kf`; it is a user module, not a built-in library key. Programs migrate from the former `import matematica` spelling to `import math_module`.
...
For the last two SVG rows, `<stem>` is exactly the unchanged suffix after `grafico_` in each old tracked filename; no other suffix substitution is implied. `src/lib/KafePLOT/utils.py` writes `<program basename>.svg`, while `tests/test_KafePLOT.py` and `tests/test_KafeGESHA.py` construct the tracked `graph_<basename>.svg` comparison paths. Keep actual generated names and comparison paths aligned. `tests/utils.py` discovers valid `*.kf` except `*.error.kf` and invalid `*.error.kf` recursively; `get_parameters` reads matching `.in` and `.expec` by base. Invalid cases additionally require full `.stderr.expec`, optional `.stdout.expec`, final semantic error, and exit code 1; valid cases require exact stdout, empty stderr, and exit code 0. Do not collapse `.error.expec` into stream sidecars or drop a case. The remaining first-party KAFE fixtures in `tests/Algorithms`, `tests/Kafe*`, `tests/base`, `tests/data`, and `tests/import` keep their path unless explicitly mapped above, but their Spanish identifiers, prose, data labels, and expected diagnostics are Task 4 content changes.
