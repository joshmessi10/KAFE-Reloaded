...
2. Check each required item against the actual repository state (not intent):
   ...
   - **Tests passed** — applicable code tasks have focused validation and the full suite (`pytest tests/`). For an approved documentation-only task with no runtime, fixture, dependency, or workflow change, record application tests as N/A with the reason and verify the approved document checks.
