...
12. Test suite: run `uv run --locked --group dev pytest tests/ -q` from the repository root (mirrors CI in `.github/workflows/tests.yml`). Report the test count and any failures.
...
- **Test results**: the `uv run --locked --group dev pytest tests/ -q` summary — number of tests passed/failed, and the first few failure names if any.
