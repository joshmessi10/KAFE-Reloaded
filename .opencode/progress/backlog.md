...
The policy baseline is integrated locally. The locked uv dependency/setup migration is implemented and locally validated on `build/uv-environment` (`f4e544a`), pushed, and verified by GitHub test CI at `b40965f` (485 passed). The interpreter subprocess quality implementation is locally complete on the user-approved `test/interpreter-quality-evidence` branch: 497 tests passed in 353.72s with 83.78% coverage and no warnings. Independent review passed; the focused commit, push, and exact-SHA CI verification remain. The branch sequence and unresolved English-migration decisions are tracked in `.opencode/progress/repository-alignment.md`; explicit authorization is required before further branch creation, rename, or switch operations.

- **Interpreter quality evidence (locally reviewed on `test/interpreter-quality-evidence`; hosted CI pending):** all 29 fixture launches in 15 test modules use the shared runner; 160 invalid fixtures have complete stderr sidecars and two have explicit stdout sidecars. The final full suite passed 497 tests in 353.72s with 83.78% coverage across 111 tracked Python source files and no warnings. The workflow now enforces the same 80% gate. Preserve CLI behavior and keep the three generated ANTLR files as the only coverage exclusions.
