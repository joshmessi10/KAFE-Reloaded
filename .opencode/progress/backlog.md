...
The policy baseline is integrated locally. The locked uv dependency/setup migration is implemented and locally validated on `build/uv-environment` (`f4e544a`), pushed, and verified by GitHub test CI at `b40965f` (485 passed). The branch sequence, scope, and unresolved English-migration decisions are tracked in `.opencode/progress/repository-alignment.md`; explicit authorization is required before creating or switching to the next planned branch.
