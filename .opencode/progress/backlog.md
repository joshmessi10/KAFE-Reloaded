...
- **Python quality gates (active on `chore/python-quality-gates`):** Task 1's approved impact and progress records are complete; implementation remains. The work will add explicit Ruff rules, basedpyright, codespell, `uv audit`, and suppression-policy checks. Warnings-as-errors and the child-interpreter coverage/diagnostic gate are implemented on `test/interpreter-quality-evidence`; the remaining tools are in the uv `dev` group. Preserve Nix lock maintenance, ANTLR prerequisites, and the docs deployment while extending the existing workflows.
